...
import { InsertUser, users, prescriptions, InsertPrescription, Prescription } from "../drizzle/schema";
...
export async function getUserPrescriptions(userId: number, limit: number = 50) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
